...
int main() {
    ...
    deque<int> overlap;
    int bestTransition[MAXN+500]; SET(bestTransition, -INF, MAXN+500);
    bestTransition[n] = 0;

    F0Rd(i, n) {
        while (sz(overlap)) {
            if (A[overlap.back()].pA >= A[i].pB) {
                int x = overlap.back();
                overlap.pop_back();
                F0R(j, k) {
                    MAX(bestTransition[x + j], dp[x][j]);
                }
            } else {
                break;
            }
        ...
        F0R(j, k) {
            dp[i][j] = bestTransition[i+j+1];

            if (sz(overlap)) {
                MAX(dp[i][j], dp[overlap.back()][max(j - (overlap.back() - i - 1), 0)] - (A[i].pB - A[overlap.back()].pA));
            }

            dp[i][j] += A[i].pB - A[i].pA;
        }
        overlap.push_front(i);
    }

    int best = 0;
    F0R(i, k) MAX(best, dp[i][k - 1 - i]);
    ps(best);
