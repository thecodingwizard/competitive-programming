...
int main() {
    ...
    for (int node : rootNodes) parent[build(node)] = -1;
        ...
        if (query.pA == 'B') {
            ...
            while (target != -1) {
                ...
                if (a.pB == -1) {
                    ...
                    if (a.pB != prevTarget) best[target].pB = { distToTarget, prevTarget };
                    else if (best[target].pA.pA < distToTarget) best[target].pA = { distToTarget, prevTarget };
