...
ll dp(int l, int r) {
    ...
    ll optimal = 0;
    FOR(k, l, r+1) {
        MAX(optimal, dp(l, k-1) + optimalWeight[l][k][r] + dp(k+1, r));
    }
    return memo[l][r] = optimal;
}
...
int main() {
    ...
    int n, m; re(n, m);
    int cowsEndingAt[n]; // max weight of cow ending at i
    vii cowsBeginningAt[n]; // <weight, endLoc>
    F0R(i, m) {
        int w, l, r; re(w, l, r);
        l--; r--;
        cowsBeginningAt[l].pb({w, r});
    }
    F0R(i, n) {
        SET(cowsEndingAt, 0, n);
        FOR(k, i, n) {
            trav(x, cowsBeginningAt[k]) MAX(cowsEndingAt[x.pB], x.pA);
            int optimal = 0;
            FOR(j, k, n) {
                MAX(optimal, cowsEndingAt[j]);
                optimalWeight[i][k][j] = optimal;
            }
        }
    }
