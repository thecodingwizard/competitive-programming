...
int main() {
    ...
    int B[n]; F0R(i, n) re(B[i]);

    int dp[(1 << 10)][(1 << 10)][11]; SET3D(dp, 0, (1<<10), (1<<10), 11);
    int dpIdx[(1 << 10)][(1 << 10)][11];
    int dpParent[n];
    ...
    F0R(i, n) {
        int best = -1, bestIdx;
        F0R(x, (1 << 10)) {
            int numOnes = __builtin_popcount(x & A[i]);
            int numOnesNeeded = B[i] - numOnes;
            if (numOnesNeeded < 0 || numOnesNeeded > 10) {
                if (best < 0) {
                    best = 0;
                    bestIdx = -1;
                }
            } else {
                int dpVal = dp[x][A[i]>>10][numOnesNeeded];
                if (best < dpVal) {
                    best = dpVal;
                    bestIdx = dpVal == 0 ? -1 : dpIdx[x][A[i]>>10][numOnesNeeded];
                }
            }
        }
        F0R(x, (1 << 10)) {
            if (dp[A[i] & ((1 << 10) - 1)][x][__builtin_popcount(x&(A[i]>>10))] < best + 1) {
                dp[A[i] & ((1 << 10) - 1)][x][__builtin_popcount(x&(A[i]>>10))] = best + 1;
                dpIdx[A[i] & ((1 << 10) - 1)][x][__builtin_popcount(x&(A[i]>>10))] = i;
                dpParent[i] = bestIdx;
                if (ans < best + 1) {
                    ans = best + 1;
                    ansState = i;
