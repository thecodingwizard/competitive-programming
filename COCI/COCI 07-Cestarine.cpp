...
int main() {
    ...
    ll dp[n+10]; SET(dp, LL_INF, n+10); dp[0] = 0;
    F0R(i, n) {
        if (A[i] != B[i]) MIN(dp[i+1], dp[i] + abs(A[i] - B[i]));
        if (i + 1 < n) {
            MIN(dp[i+2], dp[i] + abs(A[i] - B[i+1]) + abs(A[i+1] - B[i]));
        }
        if (i + 2 < n) {
            MIN(dp[i+3], dp[i] + abs(A[i] - B[i+1]) + abs(A[i+1] - B[i+2]) + abs(A[i+2] - B[i]));
            MIN(dp[i+3], dp[i] + abs(A[i] - B[i+2]) + abs(A[i+1] - B[i]) + abs(A[i+2] - B[i+1]));
        }
    }
    ps(dp[n]);
