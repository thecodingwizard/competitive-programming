...
bool operator<(dState a, dState b) {
    if (a.d == b.d) return a.ct > b.ct;
    return a.d > b.d;
}
...
void runDijkstra(int node) {
    dist[node] = 0;
    ctTo[node] = 1;
    priority_queue<dState> pq; pq.push({0, 1, node});
    while (!pq.empty()) {
        dState u = pq.top(); pq.pop();
        if (dist[u.id] < u.d || ctTo[u.id] > u.ct) continue;

        trav(e, adj[u.id]) {
            int newDist = dist[u.id] + e.len;
            if (newDist < dist[e.to]) {
                dist[e.to] = newDist;
                ctTo[e.to] = ctTo[u.id];
                pq.push({newDist, ctTo[e.to], e.to});
...
int dp(int node) {
    if (ctFrom[node] != 0) return ctFrom[node];
    int ans = 1;
    trav(e, adj[node]) {
        ...
        if (ctFrom[i] != 0) {
            trav(e, adj[i]) {
                if (dist[i] + e.len == dist[e.to]) {
                    ans[e.id] = (ans[e.id] + (ll)ctTo[i]*ctFrom[e.to]) % mod;
                }
            }
        }
    }
