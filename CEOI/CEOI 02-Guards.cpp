...
int main() {
    ...
    int n, m; re(n, m);
    int A[n][m]; F0R(i, n) F0R(j, m) re(A[i][j]);
    ii B[n][m];
    int rowCtr = n, colCtr = m;
    map<ii, ii> revMap;
    F0R(i, n) {
        F0R(j, m) {
            if (A[i][j] == 0) {
                // room
                B[i][j] = { (j==0?i:B[i][j-1].pA), (i==0?j:B[i-1][j].pB) };
                adj[B[i][j].pA].pb(B[i][j].pB);
            } else if (A[i][j] == 1) {
                // pit
                B[i][j] = { (j==0?i:B[i][j-1].pA), (i==0?j:B[i-1][j].pB) };
            } else {
    ...
    int ans = 0;
    F0R(i, rowCtr) {
        SET(vis, false, rowCtr);
        ans += aug(i);
    }
    ps(ans);
    F0R(i, colCtr) {
        if (match[i] != -1) {
            ii tgt = revMap[{match[i], i}];
            ps(tgt.pA+1, tgt.pB+1);
        }
    }
