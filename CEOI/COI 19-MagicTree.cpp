...
map<int, ll> *solve(int u) {
    auto *ans = new map<int, ll>();
    trav(v, children[u]) {
        map<int, ll> *childAns = solve(v);
        if (childAns->size() > ans->size()) {
            swap(ans, childAns);
        }
        trav(x, *childAns) {
            (*ans)[x.pA] += x.pB;
...
int main() {
    ...
    map<int, ll> *ans = solve(0);
    ll realAns = 0;
    for (auto v : *ans) realAns += v.pB;
    ps(realAns);
