...
//ll dp(int mxDigit, int num, bool und) {
//    if (num == n) return 1;
//    if (memo[mxDigit][num][und] != -1) return memo[mxDigit][num][und];
//    ll ans = 0;
//    FOR(i, 1, mxDigit+2) {
//        bool isUnd = und;
//        if (i < A[num]) isUnd = true;
//        if (!isUnd && i > A[num]) break;
//
//        ans += dp(max(mxDigit, i), num + 1, isUnd);
//        ans %= mod;
//    }
//    return memo[mxDigit][num][und] = ans;
//}
...
int main() {
    ...
    re(n); reA(A, n);
    int mx[n]; mx[0] = 0; FOR(i, 1, n) mx[i] = max(mx[i - 1], A[i - 1]);
    F0Rd(num, n+1) {
        int cur = num % 2, prv = (num + 1) % 2;

        F0R(mxDigit, num + 1) {
            F0R(und, 2) {
                if (num == n) {
                    dp[mxDigit][und][cur] = 1;
                    continue;
                }

                if (und) {
                    dp[mxDigit][und][cur] = ((ll)dp[mxDigit][und][prv]*mxDigit + dp[mxDigit + 1][und][prv]) % mod;
                } else {
                    if (mxDigit != mx[num]) continue;

                    dp[mxDigit][und][cur] = (ll)dp[mxDigit][true][prv]*min(mxDigit, A[num] - 1) % mod;
                    if (A[num] <= mxDigit) {
                        dp[mxDigit][und][cur] += dp[max(A[num], mxDigit)][false][prv];
                    } else if (mxDigit + 1 <= A[num]) {
                        dp[mxDigit][und][cur] += dp[mxDigit + 1][(mxDigit + 1) < A[num]][prv];
                    }
                    dp[mxDigit][und][cur] %= mod;
                }
            }
        }
    }
