...
int main() {
    ...
    // includes lo, doesn't include hi
    int startLo = -1, startHi = MXN, endLo = -1, endHi = MXN;
    char a1, a2;

    F0R(i, 13) {
        startHi += k; endHi += k;
        int startMid = (startLo + startHi - 1)/2; // we round *down* when searching for start
        int endMid = (endLo + endHi)/2;
        if (startLo == endLo && startHi == endHi) {
            startMid = startLo + (startHi-startLo)/3;
            endMid = endLo + (endHi-endLo)*2/3;
        }
    ...
    int maxAns = endHi - startLo + 1, minAns = endLo - startHi + 1;
