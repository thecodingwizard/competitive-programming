...
ll dp(int n, int m, int top, int left) {
    if (!top && !left) return INF;
    if (memo[n][m][top][left] != -1) return memo[n][m][top][left];

    ll best = (ll)(n*m-k)*(n*m-k);
    FOR(i, 1, n) {
        if (left == 2) {
            MIN(best, dp(i, m, top, left - 1) + dp(n - i, m, top, left - 1));
        } else if (left == 1) {
            MIN(best, dp(i, m, top, left) + dp(n - i, m, top, left - 1));
            MIN(best, dp(i, m, top, left - 1) + dp(n - i, m, top, left));
        } else {
            MIN(best, dp(i, m, top, 0) + dp(n - i, m, top, 0));
        }
    }
    FOR(j, 1, m) {
        if (top == 2) {
            MIN(best, dp(n, j, top - 1, left) + dp(n, m - j, top - 1, left));
        } else if (top == 1) {
            MIN(best, dp(n, j, top, left) + dp(n, m - j, top - 1, left));
            MIN(best, dp(n, j, top - 1, left) + dp(n, m - j, top, left));
        } else {
            MIN(best, dp(n, j, 0, left) + dp(n, m - j, 0, left));
        }
    }

    return memo[n][m][top][left] = best;
}
...
int main() {
    ...
    int n, m; re(n, m, k);
    F0R(i, n+1) F0R(j, m+1) F0R(k, 3) F0R(l, 3) memo[i][j][k][l] = -1;
    ps(dp(n, m, 2, 2));
