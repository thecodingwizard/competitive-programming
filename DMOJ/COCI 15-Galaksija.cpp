...
int p[100000], sz[100000], dRoot[100000];
vii adj[100000];
map<int, ll> ct[100000];
ll sum[100000];
ll ans = 0;

void upd(int u, int p, int xorVal, int ufIdx) {
    dRoot[u] = xorVal;
    if (xorVal == 0) {
        ans += ++ct[ufIdx][xorVal];
        sum[ufIdx] += ct[ufIdx][xorVal];
    } else {
        sum[ufIdx] += ct[ufIdx][xorVal];
        ans += ct[ufIdx][xorVal]++;
    }
...
void merge(int x, int y, int v) {
    ...
    if (sz[a] > sz[b]) return merge(y, x, v);

    ans -= sum[a];

    // merge a into b
    int xorVal = dRoot[y] ^ v;
    upd(x, y, xorVal, b);
...
int main() {
    ...
    int n; re(n);
    iii A[n-1]; F0R(i, n-1) re(A[i].pB, A[i].pA);
    int B[n-1]; reA(B, n-1);
    F0R(i, n) p[i] = i;
    F0R(i, n) sz[i] = 1;
    F0R(i, n) sum[i] = 0;

    vector<ll> print; print.pb(0);
    F0Rd(i, n-1) {
        iii e = A[B[i]-1];
        adj[e.pB.pA-1].pb({e.pB.pB-1, e.pA});
        adj[e.pB.pB-1].pb({e.pB.pA-1, e.pA});
        merge(e.pB.pA-1, e.pB.pB-1, e.pA);
        print.pb(ans);
    }
    reverse(all(print));
    trav(x, print) ps(x);
