...
int A[1000000];
vi blockedStuff[1000000];

int main() {
    setupIO("turnuri");
    InParser fin("turnuri.in");

    int n; fin >> n;
    F0R(i, n) fin >> A[i];
    vi first; first.assign(n, 0);

    deque<int> curStack;
    F0Rd(i, n) {
        while (!curStack.empty() && A[curStack.back()] < A[i]) {
            first[curStack.back()] = i;
            blockedStuff[i].pb(curStack.back());
            curStack.pop_back();
        }
    ...
    int contribute[n]; SET(contribute, 0, n);
    F0R(i, n) {
        contribute[first[i]]++;
        contribute[i]--;
    }
    FOR(i, 1, n) contribute[i] += contribute[i-1];

    F0R(i, n) {
        int idx = 0;
        while (!curStack.empty() && A[curStack.back()] < A[i]) {
            int i2 = curStack.back();
            while (idx < sz(blockedStuff[i]) && A[i2] > A[blockedStuff[i][idx]]) {
                contribute[i] -= i - i2;
                idx++;
            }
            curStack.pop_back();
        }
        while (idx < sz(blockedStuff[i])) {
            contribute[i] -= i - first[i];
            idx++;
        }
        curStack.push_back(i);
    }
    ...
    ll ans = 0;
    F0R(i, n) {
        ans += base - contribute[i] - (i - first[i]);
    }
    ps(ans);
