...
int main() {
    ...
    int m, n; re(m, n);
    int dp[1000][1000];
    int transition[1000]; SET(transition, 0, n);
    F0Rd(i, n) {
        int transition2 = 0;
        F0Rd(j, n) {
            dp[i][j] = (transition[j] + transition2 + 1) % mod;
            transition2 = (transition2 + dp[i][j]) % mod;
            transition[j] = (transition[j] + transition2) % mod;
        }
    }

    ll base = transition[0] + 1;
    ps(power(base, m-1));
