...
void dfs(int u, int p) {
    int startLoc = sz(nodes);
    nodes.pb(u);
    trav(x, adj[u]) if (x != p) dfs(x, u);
    endLoc[startLoc] = sz(nodes)-1;
}
...
int main() {
    ...
    F0Rd(i, n) {
        F0R(j, p+1) {
            if (j - A[nodes[i]] >= 0) {
                // option 1: don't block edge
                dp[i][j] = dp[i + 1][j - A[nodes[i]]];
            }

            // option 2: block edge
            dp[i][j] = (dp[i][j] + dp[endLoc[i] + 1][j]*pow2[endLoc[i]-i]) % mod;
        }
    }
