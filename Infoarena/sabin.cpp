...
string A[20000][15];
string strings[30000];
int queries[20000][15];

int main() {
    int n, k, m, p, q; fin >> n >> k >> m >> p >> q;
    F0R(i, n) {
        F0R(j, k) {
            fin >> A[i][j];
        }
    ...
    F0R(i, m) fin >> strings[i];
    F0R(i, q) {
        int x; fin >> x;
        F0R(j, k) fin >> queries[i][j];
        int maxPrefixX;
        string s;
        if (x == 0) maxPrefixX = n;
        else {
            F0R(j, x) {
                F0R(a, k) {
                    s += strings[queries[i][a]-1][j];
                }
            }
            maxPrefixX = ct[s];
        }
        int maxPrefixXplusOne;
        if (x == p) maxPrefixXplusOne = 0;
        else {
            F0R(a, k) {
                s += strings[queries[i][a]-1][x];
            }
            maxPrefixXplusOne = ct[s];
        }
        fout << maxPrefixX - maxPrefixXplusOne << "\n";
    }
