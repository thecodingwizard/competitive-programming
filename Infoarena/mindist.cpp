...
set<int> st[4*(2*MAXN)+100];

bool qry(int p, int i, int j, int l1, int r1, int l2, int r2) {
    if (j < l1 || i > r1) return false;
    if (l1 <= i && j <= r1) {
        auto it = st[p].lb(l2);
        return it != st[p].end() && *it <= r2;
    }
    return qry(p << 1, i, (i+j)/2, l1, r1, l2, r2) || qry((p << 1) + 1, (i+j)/2+1, j, l1, r1, l2, r2);
}

void upd(int p, int i, int j, int x, int y) {
    if (j < x || i > x) return;
    st[p].insert(y);
    if (i != j) {
        upd(p << 1, i, (i+j)/2, x, y);
        upd((p << 1) + 1, (i+j)/2+1, j, x, y);
    }
}
...
int main() {
    ...
    F0R(i, n) {
        int a, b; re(a, b);
        int x = a - b + MAXN, y = a + b;

        int lo = 0, hi = 2*MAXN+1, mid, ans = 0;
        while (lo < hi) {
            mid = (lo + hi)/2;
            if (qry(1, 0, 2*MAXN, x-mid,  x+mid, y-mid, y+mid)) {
                ans = mid;
                hi = mid;
            } else {
                lo = mid + 1;
