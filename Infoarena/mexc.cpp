...
int serialize(ii x) {
    return x.pB*800+x.pA;
}

int dx[4] = {-1, 0, 1, 0};
int dy[4] = {0, 1, 0, -1};
void addNode(ii node, int maxHeight) {
    F0R(i, 4) {
        int a = node.pA + dx[i], b = node.pB + dy[i];
        if (a >= 0 && a < n && b >= 0 && b < m && A[a][b] >= maxHeight) {
            unionSet(serialize(node), serialize({a, b}));
        }
    }
}

void incNode(ii node) {
    setVal[findSet(serialize(node))]++;
}
...
int main() {
    ...
    int curH = MAXH;
    F0Rd(i, MAXH+1) {
        while (curH >= 0 && i - curH <= d) {
            // union all the nodes in nodes[curH]
            trav(node, nodes[curH]) addNode(node, i - d);
            curH--;
        }

        // add +1 to set of all the nodes in nodes[i]
        trav(node, nodes[i]) {
            incNode(node);
        }

        // update the answer for all nodes in nodes[i]
        trav(node, nodes[i]) {
            ans[node.pA][node.pB] = setVal[findSet(serialize(node))];
        }
    }
