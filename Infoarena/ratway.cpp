...
int main() {
    ...
    vi oddDeg;
    F0R(i, n) {
        if (sz(adj[i]) % 2 == 1) oddDeg.pb(i);
    }

    for (int i = 1; i < sz(oddDeg); i += 2) {
        adj[oddDeg[i]].insert(oddDeg[i-1]);
        adj[oddDeg[i-1]].insert(oddDeg[i]);
        edgeCt++;
    }
    ...
    for (int i = 0; i < sz(tour); i++) {
        if (i % 2 == 0) edges.pb({tour[i], tour[(i+1)%sz(tour)]});
        else edges.pb({tour[(i+1)%sz(tour)], tour[i]});
    }
    ps(sz(edges));
    trav(x, edges) {
        ps(x.pA + 1, x.pB + 1);
