 ...
 *
 * Can eliminate i using sliding window and sliding window on k
 */
int dp[(1 << 20)][10];
int kCopy[(1 << 20)][2];
...
int main() {
    ...
    F0R(i, n) {
        while (maxSearch <= A[i]) {
            maxSearch *= 2;
            maxSearch = min(maxSearch, (1 << 20));
        }
        F0R(j, maxSearch) kCopy[j][0] = dp[j][0];
        F0R(k, d) {
            F0R(j, maxSearch) {
                kCopy[j][(k+1)%2] = dp[j][(k + 1)%d];
            }
            F0R(j, maxSearch) {
                dp[j ^ A[i]][(k + 1) % d] += kCopy[j][k%2];
                if (dp[j ^ A[i]][(k + 1) % d] >= mod) dp[j ^ A[i]][(k + 1) % d] -= mod;
            }
        }
    }
