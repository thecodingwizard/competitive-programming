...
// returns <min time, time to traverse>
pair<ll, ll> solve(int u, int p) {
    vector<pair<ll, ll>> children;
    trav(v, adj[u]) {
        if (v != p) {
            children.pb(solve(v, u));
        }
    }
    sort(all(children), [](const pair<ll, ll> x, const pair<ll, ll> y) {
        ll xBeforeY = max(1 + x.pA, 1 + x.pB + 1 + 1 + y.pA);
        ll yBeforeX = max(1 + y.pA, 1 + y.pB + 1 + 1 + x.pA);
        return xBeforeY < yBeforeX;
    });
    ll minTime = A[u], timeToTraverse = 0;
    trav(v, children) {
        timeToTraverse++;
        MAX(minTime, timeToTraverse + v.pA);
        timeToTraverse += v.pB + 1;
    }
    if (u == 0) MAX(minTime, timeToTraverse + A[u]);
    return {minTime, timeToTraverse};
}
