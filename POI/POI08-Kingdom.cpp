...
int main() {
    ...
    F0R(i, m) {
        ...
        --b;
        A[a][b] = A[b][a] = true;
        children[a].pb(b); children[b].pb(a);
    }
    ...
    // Source: https://stackoverflow.com/questions/12991758/creating-all-possible-k-combinations-of-n-items-in-c/28698654
    // Editorial also has a way to generate these combinations in an order s.t. between each combination at most 2 elements change
    int N = n - 1; // Problem specific: auto assign first city to 1
    isOne[0] = 1;
    int combo = (1 << (N / 2)) - 1;       // k = N/2 bit sets
    int cur = 0;
    F0R(i, N) {
        FOR(j, i + 1, N) {
            bool a = (combo & (1 << i)) > 0, b = (combo & (1 << j)) > 0;
            if (a != b && A[i + 1][j + 1]) cur++;
        }
        if (i < n/2) isOne[i] = 1;
    }
    FOR(i, 1, n) {
        if ((combo & (1 << (i - 1))) == 0 && A[0][i]) cur++;
    }
    int best = cur;
    int bMask = combo;
    int prevCombo = combo;
    while (combo < 1 << N) {
        F0R(i, n - 1) {
            if ((combo & (1 << i)) != (prevCombo & (1 << i))) {
                trav(j, children[i + 1]) {
                    if (isOne[i+1] == isOne[j]) cur++;
                    else cur--;
                }
                isOne[i+1] = !isOne[i+1];
            }
