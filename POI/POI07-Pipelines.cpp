...
int main() {
    ...
    sort(all(A), [](pair<ii, int> a, pair<ii, int> b) {
        if (a.pA.pA != b.pA.pA) return a.pA.pA > b.pA.pA;
    ...
    trav(x, A) {
        if (x.pB == 0) {
            int yCoord = x.pA.pB;
            auto it = s.lower_bound({yCoord, -INF});
            if (it == s.end() || it->pA > yCoord) it--;
            ans.pb(mp(mapping[x.pA], mapping[{it->pB, it->pA}]));
            s.erase(it);
        } else {
            s.insert({x.pA.pB, x.pA.pA});
        }
    }
