...
int main() {
    ...
    string n; re(n);
    // Convert from base 10 to base 4: Repeatedly divide number by 4. Its remainder each time
    // is the base 4 representation.
    int start = 0;
    while (start < sz(n)) {
        int carry = 0;
        FOR(i, start, sz(n)) {
            int num = (n[i] - '0') + carry*10;
            n[i] = '0' + num / 4;
            carry = num % 4;
        }
        number.pb(carry);
        for (; start < sz(n) && n[start] == '0'; start++);
    }

    ll xi = 0, yi = 1, Xi = 1, Yi = 1;
    F0R(i, sz(number)) {
        if (number[i] == 0) {
            Yi = Xi;
            ...
            tie(xi, yi) = mt(xi + 1, min(xi + 2, yi + 2));
        } else if (number[i] == 2) {
            if (xi < yi) {
            ...
            tie(xi, yi) = mt(min(xi + 2, yi + 2), yi + 1);
        } else {
            Xi = Yi;
