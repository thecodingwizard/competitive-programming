...
int qry(int p, int i, int j, int lb) {
    if (j < lb) return 0;
    if (i >= lb) return st[p];
    return max(qry(p << 1, i, (i+j)/2, lb), qry((p << 1) + 1, (i+j)/2+1, j, lb));
}
...
void upd(int p, int i, int j, int loc, int val) {
    ...
    upd(p << 1, i, (i+j)/2, loc, val);
    upd((p << 1) + 1, (i+j)/2+1, j, loc, val);
...
int main() {
    ...
    F0R(i, k) {
        A[i].pB.pB = yMap[A[i].pB.pB];
    }
    sort(all(A), [](const iii lhs, const iii rhs) {
        return lhs.pB.pA == rhs.pB.pA ? lhs.pB.pB > rhs.pB.pB : lhs.pB.pA > rhs.pB.pA;
    });

    int ans = 0;
    trav(x, A) {
        int xx = qry(1, 0, k - 1, x.pB.pB);
        ans = max(ans, x.pA + xx);
        upd(1, 0, k - 1, x.pB.pB, x.pA + xx);
    }
    ps(ans);
