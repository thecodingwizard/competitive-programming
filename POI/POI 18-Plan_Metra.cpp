...
int main() {
    ...
    FOR(i, 2, n-1) {
        if (abs(B[i] - A[i]) != abs(B[1] - A[1])) allSameLen = false;
    }
    if (allSameLen && B[1] - A[1] != 0) {
        int len = abs(B[1] - A[1]);
        ps("TAK");
        ps(1, n, len);
        FOR(i, 1, n-1) {
    ...
    FOR(i, 1, n-1) {
        if (A[i] + B[i] == len) {
            if (rootNodes.count(B[i] - A[i]) != 0) bad();
            rootNodes[B[i]-A[i]] = i;
        }
    }
    ...
    ps("TAK");
    auto it2 = rootNodes.rbegin();
    auto it = rootNodes.rbegin();
    it2++;
    while (it2 != rootNodes.rend()) {
        ps(it->pB+1, it2->pB+1, (it->pA - it2->pA)/2);
        it2++;
        it++;
    }
    trav(x, nodes) {
        trav(y, x.pB) {
            ps(rootNodes[x.pA]+1, y+1, B[y] - B[rootNodes[x.pA]]);
        }
    }
