...
void add(int a, int b, int v) {
    if (a == b) return;
    ans.pb(mp(mp(a+1, b+1), v));
}

vi leaves;
void getLeaves(int u, int p) {
    if (inDegree[u] == 1) leaves.pb(u);
    for (ii x : children[u]) {
        if (x.pA == p) continue;
        getLeaves(x.pA, u);
    }
...
void process(int u, int p) {
    if (p != -1) {
        ...
        int x = cost[p][u];
        getLeaves(p, u);
        int p1 = leaves[0], p2 = leaves.back();
        leaves.clear();
        getLeaves(u, p);
        int u1 = leaves[0], u2 = leaves.back();
        leaves.clear();
        add(p1, u1, x/2);
        add(p2, u2, x/2);
        add(p1, p2, -x/2);
...
int main() {
    ...
    F0R(i, n - 1) {
        ...
        children[a].pb(mp(b, v));
        children[b].pb(mp(a, v));
        cost[a][b] = cost[b][a] = v;
    }
    F0R(i, n) {
        if (inDegree[i] == 2) {
