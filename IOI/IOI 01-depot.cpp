...
int remove(int i, int j) {
    int row = i, value = A[i][j];
    int nextCol = -1;
    while (row > 0) {
        F0R(x, sz(A[row-1])) {
            if (A[row-1][x] < value) nextCol = x;
        }
        row--;
        if (nextCol == -1) return -1;
        value = A[row][nextCol];
        nextCol = -1;
    }
...
vi cur;
void solve() {
    if (sz(cur) == sz(numbers)) {
        vi tmp(cur); reverse(all(tmp)); ans.pb(tmp);
        return;
    }
    F0R(i, sz(A)) {
        if (sz(A[i]) == 1 && i != sz(A)-1) continue;
        vector<vi> tmp(A);
        int removed = remove(i, sz(A[i]) - 1);
        if (removed != -1) {
            cur.pb(removed);
            solve();
            cur.pop_back();
