...
int main() {
    ...
    re(n, k, a);
    bool isPrime[10000000]; SET(isPrime, true, 10000000);
    vi primes;
    FOR(i, 2, 10000000) {
        if (isPrime[i]) {
            primes.pb(i);
    ...
    int x = a-1;
    F0R(i, k) {
        x = forward(x, primes[i]);
    }
    int bef = (x-1+n)%n, af = (x+1)%n;
    F0Rd(i, k) {
        bef = backward(bef, primes[i]);
        af = backward(af, primes[i]);
    }
    ps(af + 1, bef + 1);
