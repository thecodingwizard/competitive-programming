...
int main() {
    ...
    while (!nodes.empty()) {
        ...
        F0R(mask, (1 << sz(children))) {
            bool works = true;
            F0R(j, sz(children)) {
                if (mask & (1 << j)) {
                    int child = children[j];
                    F0R(i, sz(children)) {
                        if ((mask & (1 << i)) && i != j && adj[child].count(children[i]) == 0) {
                            works = false;
                        }
                    }
                    if (adj[child].count(u.pB) == 0) works = false;
                }
            }
            if (works) MAX(mx, __builtin_popcount(mask)+1);
        }

        trav(child, children) {
            nodes.erase({sz(adj[child]), child});
            adj[child].erase(u.pB);
            nodes.insert({sz(adj[child]), child});
        }
    }
    ps(mx);
