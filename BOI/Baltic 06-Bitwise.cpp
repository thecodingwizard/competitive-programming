...
bool canMake(vii &exp, int i) {
    trav(e, exp) {
        if ((1LL << i) & e.pB) return true;
    }
    return false;
}

void execute(vii &exp, int i, bool done) {
    trav(e, exp) {
        if (((1LL << i) & e.pB) && ((1LL << i) & e.pA)) {
            done = true;
            e.pB -= (1LL << i);
            e.pA -= (1LL << i);
        }
...
int main() {
    ...
    vector<vii> expressions; expressions.resz(p);
    int idx = 0, ctr = 0;
    trav(x, A) {
        for (int i = 0; i < x; i++, idx++) {
            expressions[ctr].pb(B[idx]);
        }
        ctr++;
    }
    ll ans = 0;
    F0Rd(i, 31) {
        bool can = true;
        trav(exp, expressions) {
            if (!canMake(exp, i)) can = false;
        }
        trav(exp, expressions) {
            execute(exp, i, !can);
        }
